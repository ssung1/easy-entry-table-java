 ...
 * trim the contents of a sliced column. This acts as a "trimming module",
...
/**
 * This class retrieves the column name of the table, given the table and the
 * index number of the column. This is used when the easy entry table attempts to
 * continue a header line.
 */
class ColumnRetriever implements Retriever {
    /**
     * Returns the column name at position idx
     */
    public StringBuffer get(Etable table, int idx) {
        EtableColumn column;
        column = (EtableColumn) table.columnList.elementAt(idx);
        return column.name;
    }
...
/**
 * This class returns the column designated by idx from the last record of the
 * table
 */
...
public class Etable extends AbstractTable {
     ...
     * columns.
     *
     * In some cases, the columns need to be fully trimmed, but in other cases, the
     * columns need to be only right-trimmed.
     */
    ...
    /**
     * An instance of the ColumnRetriever
     */
    public static ColumnRetriever _columnRet = new ColumnRetriever();
     ...
     * HEADER line is a line that defines the columns within a table.
     */
     ...
     * The column widths and offsets are set to zero
     *
     * @return column list
     */
     ...
     * column.
     */
    public int _createColumnLst(String buf) {
        columnList = createColumnLst(buf);
        return columnList.size();
    }
     ...
     */
    public static Vector<Column> createColumnLst(String buf) {
        /*
        ...
        Vector<Column> column_lst = new Vector<>();
         ...
         */
        EtableColumn column = null;
        for (int idx = 1; idx <= buf.length(); idx++) {
            char chr;

            /*
             * this is a workaround. we need to make sure the last column * gets registered,
             * so we are appending a space character to flush out the last column 
             */
            if (idx < buf.length()) {
                chr = buf.charAt(idx);
            } else {
            ...
            switch (status) {
                case 100:
                    if (!Character.isWhitespace(chr)) {
                        /*
                         * Update information for the previous column, if there is one
                         *
                         * There must not be any gap between two columns. This is done for the
                         * convenience of the user.
                         */
                        if (column != null) {
                            column.setSize(idx - column._offset);
                        }

                        /*
                         * make a new column now
                         */
                        column = new EtableColumn();
                        column._offset = idx;

                        column.name.append(chr);
    ...
    public int pushLine(String buf) {
        ...
        switch (buf.charAt(0)) {
            ...
            case ' ':
                 ...
                 * appending a record onto the column names.
                 */
    ...
    public int pushCont(String buf) {
        switch (_prev_status) {
            case HEADER:
                slice(buf, _columnRet, _rtrimmer);
                return 1;
            case RECORD:
                /*
    ...
    /**
     * Cuts a string into slices according to the column definitions.
     *
    ...
    public List<StringBuffer> _slice(String buf, Retriever retriever, StringTrimmer trimmer) {
        ...
        for (idx = 1; idx < columnList.size(); idx++) {
             ...
             */
            column = (EtableColumn) columnList.elementAt(prev_idx);
             ...
             */
            if (column._offset >= len) {
                continue;
            }

            int endidx = column._offset + column._size;
            /*
             * Compare this with the previous statement. We only check for "greater than"
             * because it is okay if the offset of the NEXT column is out of range.
             */
            if (endidx > len) {
                newpiece = buf.substring(column._offset);
                newpiece = trimmer.trim(newpiece);
                piece.append(newpiece);
                continue;
            }
            newpiece = trimmer.trim(buf.substring(column._offset, endidx));
            piece.append(newpiece);
        }

        /*
         * The last column is a special variable length column.
         *
         * idx after the loop is equal to columnList.size(), so we get the element at
         * idx - 1
         */
        piece = retriever.get(this, idx - 1);
        column = (EtableColumn) columnList.elementAt(idx - 1);

        pieces.add(piece);
        if (column._offset >= len)
            return pieces;

        newpiece = trimmer.trim(buf.substring(column._offset));
        piece.append(newpiece);
